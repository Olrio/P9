...
def flux(request):
    ...
    """
        ...
        such a queryset is needed to know if a review
        is possible on an autoreview.
        If the ticket of the autoreview is present in this queryset,
        review on this ticket would be disable
    """
    reviews_not_autoreview = Review.objects.filter(
        ...
        | Q(user=request.user)
        | Q(ticket__user=request.user)
    ).exclude(Q(user=F("ticket__user")))

    """
    a queryset of all autoreviews, i.e. review and ticket have the same author
    """
    ...
    tickets = Ticket.objects.filter(
        ...
        | Q(user=request.user)
    ).exclude(Q(id__in=[review.ticket.id for review in autoreviews]))
    ...
    context = {"page_obj": page_obj, "reviewable": list(reviewable_tickets)}
...
def subscribe(request):
    ...
    users_to_follow = User.objects.all().exclude(
        Q(username__in=[follow.followed_user for follow in followed_users])
...
def unsubscribe(request, user_id):
    follow = get_object_or_404(UserFollows, id=user_id)
    if request.method == "POST":
        follow.delete()
        return redirect("subscribe")
...
def ticket_create(request):
    ...
    return render(request, "critics/ticket_create.html", {"form": form})


@login_required()
def ticket_read(request, ticket_id):
    ticket = get_object_or_404(Ticket, id=ticket_id)
    return render(request, "critics/ticket_read.html", {"ticket": ticket})
...
def review_create(request):
    ...
    if request.method == "POST":
        ...
        if all([ticket_form.is_valid(), review_form.is_valid()]):
            ...
            return render(
                request,
                "critics/review_read.html",
                {"ticket": ticket, "review": review},
            )
    return render(
        request,
...
def review_read(request, review_id):
    ...
    return render(request, "critics/review_read.html", {"review": review})
...
@login_required()
def review_answer_ticket(request, review_id):
    ticket = get_object_or_404(Ticket, id=review_id)
    review_form = ReviewForm()
    if request.method == "POST":
